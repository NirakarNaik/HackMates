...
import { intersect, norm, normalizeArray, expandToSkills } from "./utils.js";
...
function skillComplementarity(a, b) {
  ...
  const bProvides = intersect(expandToSkills(normalizeArray(b.skills)), aNeedsExpanded);
  const aProvides = intersect(expandToSkills(normalizeArray(a.skills)), bNeedsExpanded);

  const ratioForA = needCoverageRatio(a.looking_for, b.skills);
  const ratioForB = needCoverageRatio(b.looking_for, a.skills);
